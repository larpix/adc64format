...
def test_skip_file(example_file):
	...
	# get first event
        ...
        assert first_event['event'][0]['serial']+nskip+1 == event['event'][0]['serial']
...
def test_skip_multi_file(example_files):
    with ADC64Reader(*example_files) as reader:
        ...
        first_event = []
        for f in reader._fs:
            first_event.append(parse_chunk(f))
...
def test_read_file(example_file):
    ...
    with ADC64Reader(example_file) as reader:
        while event := reader.next(batch_size):
            # check that we return correct type
            assert isinstance(event, dict), 'returned type is incorrect'
